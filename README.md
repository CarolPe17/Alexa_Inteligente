...
<sub>Le hablas normal, te sigue el hilo, le cambias de tema sin repetir "Alexa"… y cuando te despides, se calla solito 🫶</sub>

<br/><br/>

<img alt="Alexa" src="https://img.shields.io/badge/Alexa-Skill-FFAFCC?style=for-the-badge&logo=amazonalexa&logoColor=white" />
<img alt="Azure OpenAI" src="https://img.shields.io/badge/Azure_OpenAI-gpt--4.1--mini-CDB4DB?style=for-the-badge&logo=microsoftazure&logoColor=white" />
<img alt="Node" src="https://img.shields.io/badge/Node-%3E%3D18-FFC8DD?style=for-the-badge&logo=node.js&logoColor=white" />
<img alt="Express" src="https://img.shields.io/badge/Express-5-BDE0FE?style=for-the-badge&logo=express&logoColor=white" />
<img alt="voz" src="https://img.shields.io/badge/voz-es--MX-FCCEEE?style=for-the-badge" />
<img alt="hecho con amor" src="https://img.shields.io/badge/hecho_con-%F0%9F%92%96-FB6F92?style=for-the-badge" />
...
- 🧠 **Con memoria** — recuerda lo que se dijo antes y le da continuidad (le dices *"¿y eso por qué?"* y sabe de qué hablas 🩷).
- 👋 **Cierre natural** — detecta cuando te despides (*"gracias, adiós"*) y termina la sesión de verdad, sin bucles raros.
- ⚡ **Nunca se siente muerto** — suelta una muletilla (*"Claro", "Va", "A ver"*) mientras piensa, para no cortar por el límite de ~8s de Alexa.
...
```mermaid
%%{init: {'theme':'base', 'themeVariables': {'primaryColor':'#ffc8dd','primaryTextColor':'#7a4a63','primaryBorderColor':'#ff8fab','lineColor':'#ff8fab','secondaryColor':'#cdb4db','tertiaryColor':'#fdeff4','fontFamily':'Segoe UI'}}}%%
flowchart LR
    A["🗣️ Tu voz"] --> B["Alexa"]
...
Tres detalles hacen la magia ✨

| Pieza | Qué hace |
|---|---|
| 🎀 **Slot libre** (`Dialog.ElicitSlot`) | Alexa toma lo *siguiente* que digas como la pregunta completa, sin carrier phrases. Necesita un `dialog model` en el modelo. |
| 🧠 **Memoria por sesión** | Historial de la charla en memoria, por `sessionId`, inyectado en cada respuesta. |
| 👋 **Marca de despedida** | El modelo agrega `[FIN]` cuando la persona se despide; ahí se cierra la sesión. |
...
El modelo define el `invocationName` (`jarvis`), el intent `AskIntent` con un slot `query`
(`AMAZON.SearchQuery`) y el `dialog` model con `delegationStrategy: SKILL_RESPONSE` — sin ese
`dialog`, el directivo `ElicitSlot` no es válido y Alexa cierra la sesión 🙈.
...
<div align="center">
<sub>˚₊‧ hecho con 💖 para que la Alexa deje de ser tan tonta ‧₊˚</sub>
</div>
